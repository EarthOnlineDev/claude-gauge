...
- **刷新** —— 一个后台任务，从 Claude Code `/usage` 同源的接口取你的用量，用 Claude Code 已经存在钥匙串里的 token 鉴权。它**自适应节流**：宽裕时几乎不打扰接口，临近限额才加快，离限流远远的。它还能**自己续命**：Claude Code 闲置时那个 token 会过期，刷新器在它快过期时悄悄让 Claude Code 续上（一条只占一个 token 的提示，只在需要时才发，几乎不耗额度），表盘从不"罢工"。
