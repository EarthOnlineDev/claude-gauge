...
def title_line(fh,wk,d,stale=False,armed=False):
    ...
    if armed:
        rb=ICON_RAINBOW_DARK if _is_dark() else ICON_RAINBOW   # 深色菜单栏用白针变体，避免黑针糊
        act=f"image={rb} {ICON_SZ} bash=/usr/bin/python3 param0={ALERT} param1=open terminal=false"
