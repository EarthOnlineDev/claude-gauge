...
# 当前登录凭证的指纹 —— 显示任何带身份戳的缓存前，必须核对它属于【当前这个账号】
_cur_tk=read_token()
_cur_fp=cred_fp(_cur_tk.get("accessToken")) if _cur_tk else None
def _belongs(c):
    """这份缓存能不能显示给当前用户：无 cred 戳(live.json/旧格式)→放行；
    有戳但读不到本机凭证→不显示(宁可不显示也不显示别人的)；有戳→必须等于当前凭证指纹。"""
    if not c: return False
    if "cred" not in c: return True
    if _cur_fp is None: return False
    return c.get("cred")==_cur_fp
best=None
...
if (best is None) or (time.time()-best["ts"]>150):   # 兜底：后台失效才自己拉
    ...
    if tk and ((tk.get("expiresAt") is None) or (tk["expiresAt"]/1000>time.time()+30)):
        try:
            ...
            # 只在 org.json 确属当前凭证(或旧格式无戳)时才用它的 uuid，绝不拿别账号的 org 去拉
            try:
                _oj=json.load(open(org_f))
                if _oj.get("fp") in (None,_cur_fp): org_uuid=_oj.get("uuid")
            except Exception: pass
            ...
            obj={"ts":time.time(),"data":dd,"org":org_uuid,"cred":cred_fp(tk["accessToken"])}; json.dump(obj,open(CACHE,"w")); best=obj
        except Exception: pass
if best is None:
