...
/usr/bin/python3 <<'PY'
import os, json, urllib.request, urllib.error, datetime, time, sys, subprocess, tempfile, hashlib
def cred_fp(access_token):
    """凭证指纹：accessToken 的单向哈希(前16位)。把缓存绑定到具体登录账号，绝不存/打印明文。
    必须与 refresher 里同名函数完全一致。"""
...
ORGJSON=os.path.expanduser("~/.cache/claude-gauge/org.json")        # 本机刷新器为当前账号写的权威 org，用于核对缓存归属
...
# 缓存归属核对（隐私红线）：只显示【由当前钥匙串里这份 token 取得】的数据。
# 每份缓存/实时文件都盖 fp=写入时 token 的指纹；这里现算当前 token 指纹，只认 fp 相等的。
# fp 恒可算、不依赖网络：换号(fp 不同)、别账号同步进来的缓存、token 轮换后的旧数据，fp 必不符 → 一律拒显、回退现拉。
_MYTK=read_token()
_MYFP=cred_fp(_MYTK.get("accessToken")) if _MYTK else None
def _belongs(c):
    """能否显示：必须带 fp 戳且等于当前 token 指纹。无 fp 戳(旧格式/未更新的 bridge)无法证明归属 → 不显示，
    改用当前 token 现拉(升级后一次过渡，之后缓存都带 fp)。"""
    if not c or _MYFP is None: return False
    return c.get("fp")==_MYFP
best=None
for c in (load(LIVE),load(CACHE)):
    if _belongs(c) and (best is None or c["ts"]>best["ts"]): best=c
if (best is None) or (time.time()-best["ts"]>150):   # 兜底：后台失效才自己拉
    tk=_MYTK                                          # 复用顶部已读的当前 token，避免二次读钥匙串
    if tk and ((tk.get("expiresAt") is None) or (tk["expiresAt"]/1000>time.time()+30)):
        try:
            org_uuid=None
            # 只在 org.json 的 fp 【确等于当前 token】时才用它的 uuid；无 fp(旧格式)或不符一律不信 → 不带 org 头
            try:
                _oj=json.load(open(ORGJSON))
                if _oj.get("fp") is not None and _oj.get("fp")==_MYFP: org_uuid=_oj.get("uuid")
            except Exception: pass
            ...
            # 盖 fp=当前 token 指纹（这份就是用当前 token 拉的）→ 下次本插件/刷新器据此认得是本账号数据
            obj={"ts":time.time(),"data":dd,"fp":_MYFP,"org":org_uuid}
            _atomic(CACHE,obj); best=obj                 # 原子写，避免半截文件被并发读到(修 L3)
        except Exception: pass
if best is None:
