...
核心三层 + 两个可选层（statusLine 桥接 / 完成提醒），彻底解耦：**写数据的不管渲染，渲染的不管拉取**。
...
| 提醒层（可选 · opt-in） | `alert/claude-gauge-alert.py` | Claude Code 的 `Stop` / `Notification(permission_prompt)` hook + 菜单栏左键点击 | 事件触发时记一个时间戳写 `attention.json`、点击拉起 Claude 写 `ack.json`，让菜单栏表盘亮「有新发现」彩虹态 | 否（仅本机 hooks，**绝不读对话/代码/transcript**） |
| 安装/卸载 | `install.sh` / `uninstall.sh` | 手动 | 装组件、配 LaunchAgent、提示 statusLine；卸载只删自己 | 否 |

> **提醒层（第 4 层）是独立 opt-in 模块**，由专门的 `alert/install-alerts.sh` 安装，主 `install.sh` 不碰它；默认不装。下文 §2、§3、§4、§5、§6 描述的是核心三层 + 桥接层；提醒层的完整契约与渲染逻辑见 **§8.5**。
...
- **提醒层是可选的"离场提醒"**。装了它（`alert/install-alerts.sh`）之后，Claude Code 自己的「一个回合完成 / 停下来等授权」hook 事件会触发它往 `attention.json` 写一个**时间戳**；若事件发生时你不在 Claude 桌面 App 前台，渲染层就把表盘点成彩虹叫你回来，左键点一下拉起 Claude、彩虹熄灭。它**只对 CC 推送的事件作反应，绝不读对话或代码**，不联网、不弹系统通知、零遥测；不装则 `attention.json` 不存在、相关分支全程短路，菜单栏输出与今天逐字节一致（详见 §8.5）。
...
| `alert/claude-gauge-alert.py` | 提醒层（可选 · opt-in）事件入口：被 CC 的 `Stop`/`Notification` hook 与菜单栏左键点击调用，写 `attention.json`/`ack.json`（详见 §8.5） |
| `alert/install-alerts.sh` | 提醒层 opt-in 装/卸：把 Stop + Notification(permission_prompt) hook **幂等**合并进 `~/.claude/settings.json`（先备份、回解析校验、原子写、只删自己）；`--uninstall` 反向移除 |
...
| `~/.claude/claude-gauge-alert.py` | 提醒层副本（可选 · opt-in） | alert/install-alerts.sh | CC 的 `Stop`/`Notification` hook、菜单栏左键点击 |
...
| `~/.cache/claude-gauge/attention.json` | 提醒层未读事件 `{ts, event, front, host}`（可选 · 装提醒层才有） | 提醒层（hook 触发 `event` 时） | 渲染层 |
| `~/.cache/claude-gauge/ack.json` | 提醒层已读标记 `{ts}`（可选 · 装提醒层才有） | 提醒层（左键点击）＋渲染层（检测到会话载体回前台时） | 渲染层 |
...
### 8.5 完成提醒 / 彩虹层（可选 · opt-in）

「有新发现」是一个**可选的第 4 层**：你离开本次会话的**载体 App**（运行 CC 的终端 App，或 Claude 桌面 App）后，若有 Claude Code 会话**完成**（`Stop`）或停下来**等你授权**（`Notification` 的 `permission_prompt`），菜单栏表盘点成**彩虹**叫你回来；**左键点图标 = 拉回该会话所在的载体（终端会话→终端 App，桌面会话→桌面端）+ 熄灭彩虹**。它**只对 CC 自己推送的 hook 事件作反应——绝不读对话、代码、transcript**；只读进程元数据（`lsappinfo`/`ps`/`defaults`）认载体，不弹辅助功能/自动化授权框；不联网、不弹系统通知、零遥测。默认不装；**不装时 `attention.json` 不存在 → 下面整套逻辑全程短路，菜单栏输出与今天逐字节一致**。
...
未装该层时 `attention.json` 不存在，`_armed()` 恒 `False`、`render()` 里相关分支不进，零额外开销。
...
- **触发源**：仅 `Stop`（一个回合完成）+ `Notification`/`permission_prompt`（卡住等授权），**不含 idle**（与"完成"重复）。只关注「有/无」未读，不区分是哪个 / 几个会话。
- **opt-in 装/卸**：`bash alert/install-alerts.sh` 装 alert 脚本并把两条 hook **幂等**合并进 `~/.claude/settings.json`（`Stop` → `event stop`；`Notification` matcher `permission_prompt` → `event permission`）——改前先备份、回解析校验、原子写、**绝不动用户已有的 hooks**。`bash alert/install-alerts.sh --uninstall` 只删 command 含 `claude-gauge-alert.py` 的条目（空组 / 空事件不留空壳），其余 hook 原封不动。这是唯一会自动改 `settings.json` 的入口，主 `install.sh`/`uninstall.sh` 都不碰它。

### 8.6 菜单卸载入口 + 稳定卸载脚本

为了让用户不必回到 clone 目录就能卸载，`install.sh` 把卸载脚本装到一个**与 clone 解绑的稳定位置**，并在下拉里挂一个收纳子菜单：

- **稳定副本**：`install.sh:29` 用 `install -m 0755 "$REPO/uninstall.sh" "$HOME/.claude/claude-gauge-uninstall.sh"`——即使 clone 目录被删/移动，卸载脚本仍在。`uninstall.sh` 结尾 `rm -f "$HOME/.claude/claude-gauge-uninstall.sh"`（`uninstall.sh:13`）自删这份副本，卸得干净。
...
- **仍保持克制**：主卸载脚本**不碰** `settings.json` 的 hooks、**不删** `claude-gauge-alert.py`，只在输出里提示用户 `bash alert/install-alerts.sh --uninstall` 单独移除完成提醒层（`uninstall.sh:12`）。statusLine 同样提示用户自行移除（`uninstall.sh:11`）。
...
# attention.json / ack.json ← 提醒层未读/已读（装了完成提醒层才有；attention 含 host=会话载体）
...
- 可选的**提醒层**（§8.5）只对 CC 自己推送的 `Stop`/`Notification` hook 事件作反应、只记一个时间戳 + 事件名 + 前台/会话宿主 App 的 bundle id（**面向会话载体 ⑤** 只读进程元数据 `lsappinfo`/`ps`/`defaults`，不弹辅助功能/自动化授权框），**绝不读 stdin/transcript/对话/代码**；纯本机 hooks，不联网、不弹系统通知、零遥测。
- **无遥测**；token 只发往 Anthropic，不发往任何第三方。
- 全部是**可读的 bash / python**，无混淆、无编译产物，可逐行审计。
- 卸载（`uninstall.sh`）只删自己装的东西，**不碰** Claude Code 的凭证与数据，statusLine 配置需用户自行从 `~/.claude/settings.json` 移除（`uninstall.sh:11`）。
