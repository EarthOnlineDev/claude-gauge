...
关键约定：**`live.json` 与 `cache.json` 结构完全一致**——都是 `{"ts": <epoch 秒>, "data": {...}}`。渲染层只比较两者的 `ts`，谁新用谁（`plugin/claude-gauge.15s.sh:131-133`）。这就是"plugin 读较新者"的全部含义：你用 CC 时桥接层写得更勤，菜单栏跟着 CC 实时刷新；你不用 CC 时数据层每分钟级别更新。
...
阈值常量在 `plugin/claude-gauge.15s.sh:19`：`WARN_TH, CRIT_TH = 25.0, 10.0`（剩余值口径）。分级函数 `_lvl(p)` 在 `:55-59`：`p<=10 → 2`、`p<=25 → 1`、否则 `0`。**配色刻意去绿**——够用时不是绿色而是近黑/近白，把颜色这个强信号留给真正需要注意的时刻。

### 4.3 菜单栏标题状态机（`title_line`，`plugin/claude-gauge.15s.sh:68-87`）
...
- **5 小时窗口无前缀**（即"当前"），**周窗口加 `W` 前缀**——`s5()` 与 `s7()` 函数，`:75-76`。
- **额外用量提示**：若 `extra_usage.is_enabled` 且 `used_credits > 0`，且当前是够用状态、加上后宽度仍 ≤ `MAXW`，标题追加 `+$`（`:84`）。
- 倒计时格式：5h 用 `_cd5`（分钟/`{h}h{mm}m`，≥10 小时封顶显 `9h+`，`:39-46`）；周用 `_cd7`（`{d}天{h}时` / `{d}天` / 小时 / 分钟，`:47-54`）。

### 4.4 宽度硬约束（刘海）

带刘海的 Mac，菜单栏标题一旦超过约 90px（≈ 11 字符）就会被刘海吞掉**整个消失**。所以：

- `MAXW = 11`（`plugin/claude-gauge.15s.sh:26`）是字符宽度上限。
- 宽度用 `_w(s)` 计算（`:60`），中文字符（`ord(c) > 0x2E80`）按 2 宽度算，其余按 1。
- `+$` 提示只在加上后仍不超宽时才追加（`:84`）。
- 倒计时封顶常量 `CD_CAP = "9h+"`（`:26`），避免长倒计时撑爆宽度。

### 4.5 陈旧检测（诚实陈旧）

- `STALE_SEC = 900`（15 分钟，`:18`）。
- `render()` 里 `age = time.time() - ts; stale = age > STALE_SEC`（`:96`）。
- 陈旧时：菜单栏标题变灰加 `~`（`:85`），且下拉菜单插入警告块：`⚠️ 数据已 N 分钟未更新` + `闲置/限流；用一下 Claude Code 即刷新`（`:102-105`）。
- 这样陈旧数据一眼可辨，菜单栏不会停在过时的数字上。

### 4.6 下拉菜单结构（`render` + `section`，`:89-116`）

SwiftBar 的坑：一行若**既无点击动作又无颜色**，会被渲染成"禁用灰"。所以**每一行都显式上色**。

每个窗口由 `section(label, icon, u, cd_str, col)` 渲染三/四行（`:89-93`）：
...
进度条函数 `bar(used)`（`:29-30`）：`f = round(used/10)`，输出 `"█"*f + "░"*(10-f)`。

下拉还包含：标题行 `Claude Code 用量`；按模型分项（若有 `seven_day_sonnet` / `seven_day_opus`，`:109-112`）；更新时间 `更新于 HH:MM（N分钟前/刚刚）`（`:114-115`）；以及一个可点击的 **`立即刷新（强制拉最新）`** 项——它 `shell` 调用 `~/.claude/claude-gauge-refresh.sh` 并传 `param0=force`，强制数据层立刻 poll（`:116`）。
...
渲染层也能读 token 自己拉一次（`read_token`，`plugin/claude-gauge.15s.sh:122-129`），但**仅在后台缓存彻底失效时**才触发——`best is None` 或 `best.ts` 比当前早超过 150 秒（`:134`）。它读 token 前会检查 `expiresAt` 是否还有 30 秒余量（`:136`），过期就不拉。注意渲染层**不做 token 续命**，续命是数据层的专责。
...
- **渲染层显示分级**：`plugin/claude-gauge.15s.sh:19` 的 `WARN_TH, CRIT_TH = 25.0, 10.0`（剩余值口径，对应已用 75% / 90%）。改这里会改菜单栏的橙/红切换点。
...
`plugin/claude-gauge.15s.sh:20`：`COL_WARN="#e08a2b"`（橙）、`COL_CRIT="#e0483d"`（红）、`COL_STALE="#9a9a9a"`（灰）。够用色在 `:24` 的 `NORMAL`（深色 `#ededef` / 浅色 `#1d1d1f`，由 `_is_dark()` 判定）。辅助灰 `MUTE` 在 `:25`。配色"去绿"是刻意设计，改时建议保持够用态为低饱和度中性色。
...
- **渲染层兜底阈值**：`plugin/claude-gauge.15s.sh:134` 的 `150` 秒（best 多久没更新才让渲染层自己拉）。

### 8.4 加模型分项

数据层已经在抓 `seven_day_sonnet` / `seven_day_opus`（`refresher/claude-gauge-refresh.sh:79` 的循环 key 列表），渲染层也已展示（`plugin/claude-gauge.15s.sh:98`、`:109-112`）。要加新模型分项：

1. 在数据层 `:79` 的 key 元组里加上新窗口名（前提是 API 返回该 key）。
2. 在渲染层 `render()` 里仿照 `son`/`opus` 取 `remain(...)` 并加进 `extras` 列表（`:109-112`）。
