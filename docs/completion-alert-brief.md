...
- **形态**：第 4 个**可选层**（opt-in），默认不开；**没装这层的用户，菜单栏与现在逐字节一致**。
...
- **零遥测、零第三方、零自有服务器**。
- **opt-in**：要多跑一条命令装这一层（它会把 Stop/Notification hook 合并进你的 `~/.claude/settings.json`，**已做：先备份 → 幂等 → 校验 → 外科式卸载，绝不动你已有的 hooks**）。默认不开。
...
- 诚实小字：`Opt-in. It reacts only to Claude Code's own "finished / needs-permission" signals — never your conversations or code. No pop-ups, no telemetry.` / `可选开启。只对 Claude Code 自己的「完成 / 等授权」信号作反应——绝不读你的对话或代码。无弹窗、无遥测。`
...
- 标号 `04 · ALERT / 提醒`，`optional/可选` 徽章
- h3：`The nudge` / `提醒`，tag：`alert/claude-gauge-alert.py`
- p：`An opt-in layer that hooks Claude Code's own "finished" and "needs-permission" events, records just a timestamp (never the transcript), and flips the gauge to rainbow when you're away. Click to raise Claude and clear it.` / `一个可选层，挂上 Claude Code 自己的「完成」「等授权」事件，只记一个时间戳（绝不读 transcript），在你离开时把表盘翻成彩虹。点一下拉起 Claude 并熄灭。`
...
- 价值一句话（见 §0）。
- **opt-in 安装**：`bash alert/install-alerts.sh`（卸载 `bash alert/install-alerts.sh --uninstall`）。
...
- **安装**：`alert/install-alerts.sh` —— 装 `claude-gauge-alert.py` 到 `~/.claude/`，并把两条 hook 幂等合并进 `~/.claude/settings.json`：`Stop`（→ event stop）、`Notification` matcher `permission_prompt`（→ event permission）。改前备份、回解析校验、原子写、**不碰用户已有 hooks**。
- **卸载**：`alert/install-alerts.sh --uninstall` —— 只删 command 含 `claude-gauge-alert.py` 的条目。
